...
const TicketCartPage: React.FC = () => {
  ...
  return (
    <div className="bg-gray-100 min-h-screen">
      <Header />
      <div className="bg-gray-100 min-h-screen p-4 sm:p-8 mt-8">
        <div className="flex flex-col lg:flex-row justify-center items-start space-y-8 lg:space-y-0 lg:space-x-8">
          ...
          <div className="bg-white p-6 rounded-xl shadow-lg w-full lg:w-1/2 flex flex-col h-auto lg:h-[31.7rem] overflow-y-auto">
            <div className="mb-4">
              <h3 className="text-2xl font-bold cursor-pointer" onClick={() => setShowRegularTicket((prev) => !prev)}>
                Regular Ticket
                <span className="ml-2 text-blue-500 text-sm">{showRegularTicket ? '' : ''}</span>
              </h3>
            </div>
            <div className={`overflow-hidden transition-all duration-300 ease-in-out ${showRegularTicket ? 'max-h-[1000px] opacity-100' : 'max-h-0 opacity-0'}`}>
              <div className="border rounded-md p-4 mb-4">
                ...
                <div className="flex items-center mt-4">
                  ...
                  />
                  <button className="bg-blue-500 text-white rounded-r-md px-2 sm:px-4 py-2 hover:bg-blue-600 transition-colors duration-200" onClick={() => alert('Coupon applied!')}>
                    Apply
                  </button>
                </div>
              </div>
            ...
            <div className="flex justify-between items-center mt-4">
              <div>
                <p className="text-base sm:text-lg font-bold">Total: ₹{calculateTotal().toFixed(2)}</p>
                <p className="text-xs sm:text-sm text-gray-500">Excl. all taxes</p>
              </div>
              <button
